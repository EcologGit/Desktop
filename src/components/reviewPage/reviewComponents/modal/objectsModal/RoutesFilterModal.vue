<template>
  ...
  >
    ...
    <div style="display: flex; gap: 32px; flex-direction: column">
      <div style="display: flex; gap: 16px; flex-direction: column">
        <p class="label-parameter">Длина маршрута, км</p>
        <div class="input-range-button-block">
          <input
            placeholder="От"
            class="search-parameter"
            type="number"
            v-model="filters.length_greater_or_equal"
          />
          <input
            placeholder="До"
            class="search-parameter"
            type="number"
            v-model="filters.length_less_or_equal"
          />
        </div>
      </div>
    </div>
...
const resetFilter = () => {
  filters.value = {
    report_count: "no_matter",
    admarea_name: "",
    length_greater_or_equal: null,
    length_less_or_equal: null,
  };
};
